...
class _DialogueBankTransactionState extends State<DialogueBankTransaction> {
  ...
  // Client types
  final List<String> clientTypes = ['Individual', 'Establishment'];
  ...
  bool _matchesClientType(Map<String, dynamic> project, String clientType) {
    // Get the client type from the project data
    final projectClientType = project['client_id']?['client_type']?.toString().toLowerCase() ?? '';

    switch (clientType.toLowerCase()) {
      case 'individual':
        return projectClientType.contains('individual') || projectClientType.isEmpty;
      case 'establishment':
        return projectClientType.contains('establishment');
      default:
                  ...
                  children: [
                    ...
                    Expanded(
                      child: CustomDropdownField(
                        ...
                        onChanged:
                            (value) =>
                                setState(() => selectedPaymentType = value == 'Select payment type' ? null : value),
                      ),
                    ),
                    const SizedBox(width: 10),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: CustomDropdownField(
                        ...
                        options: ['Select Client Type', ...clientTypes],
                        onChanged: (value) {
                          setState(() {
                                ...
                                setState(() {
                                  ...
                                  if (value != null && value != 'Select Project') {
                                    ...
                                    if (selectedProjectData != null &&
                                        selectedProjectData!['client_id']?['name'] != null) {
                          ...
                          builder: (context, projectStageProvider, child) {
                            ...
                            return CustomDropdownField(
                              ...
                              options: [
                                'Select Stage',
                                ...activeStages.map(
                                  (stage) => '${stage['service_department'] ?? 'Unknown Department'}',
                                ),
                              ],
                              onChanged: (value) {
                                setState(() {
                                  ...
                                  if (value != null && value != 'Select Stage') {
                                    ...
                                    // Auto-fill additional profit if available
                                    if (selectedStageData != null && selectedStageData!['additional_profit'] != null) {
                                      _additionalCostController.text =
                                          selectedStageData!['additional_profit'].toString();
                                    }
                                  } else {
                                    selectedStageData = null;
                                  }
                          ...
                          builder: (context, documentsProvider, child) {
                            ...
                            return Column(
                              children: [
                                GestureDetector(
                                  onTap:
                                      (documentsProvider.isUploading || _isProcessing || hasAttachedFile)
                                          ? null
                                          : () {
  ...
  void _submitForm() async {
    ...
    if (selectedPaymentType == 'Receive' && selectedProjectStage == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Please select project stage for Receive payment type')));
      return;
    }
    ...
    if (selectedPaymentMethod == 'Bank' && selectedBank == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Please select a bank for bank payment')));
      return;
    }
    ...
    final stepCostVal =
        (selectedPaymentType == 'Expense' && _stepCostController.text.isNotEmpty)
            ? double.tryParse(_stepCostController.text)
            : null;

    final additionalProfitVal =
        (selectedPaymentType == 'Expense' && _additionalCostController.text.isNotEmpty)
    ...
    if (bankingProvider.successMessage != null && bankingProvider.successMessage!.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(bankingProvider.successMessage!)));
      _clearForm();
      if (Navigator.canPop(context)) {
        Navigator.of(context).pop();
      }
    } else if (bankingProvider.errorMessage != null && bankingProvider.errorMessage!.isNotEmpty) {
  ...
  void _updatePayment() async {
    ...
    if (selectedPaymentType == 'Receive' && selectedProjectStage == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Please select project stage for Receive payment type')));
      return;
    }
    ...
    final double? stepCostUpdateVal =
        (selectedPaymentType == 'Expense' && _stepCostController.text.isNotEmpty)
            ? double.tryParse(_stepCostController.text)
            : null;
    final double? additionalCostUpdateVal =
        (selectedPaymentType == 'Expense' && _additionalCostController.text.isNotEmpty)
            ? double.tryParse(_additionalCostController.text)
            : null;
                          ...
                          Container(
                            ...
                            child: Center(
                              child: Column(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
                                  Icon(_getFileIcon(), size: 48, color: Colors.grey.shade400),
  ...
  void _removeSelectedFile() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('File removed'), backgroundColor: Colors.orange, duration: Duration(seconds: 2)),
    );
  }
  ...
  Widget _buildDocumentItem({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isAttached ? Colors.orange.shade300 : Colors.grey.shade300),
        borderRadius: BorderRadius.circular(6),
      ),
      child: Row(
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (isAttached)
                  Text('Attached (Not Uploaded)', style: TextStyle(fontSize: 12, color: Colors.orange.shade700))
  ...
  Future<void> _deleteDocument(String documentRefId) async {
    ...
    if (isAttached) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Attached document removed')));
    } else {
