...
class _ShortServiceScreenState extends State<ShortServiceScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    print('ShortServiceScreen rebuild - isLoading: ${shortServicesProvider.isLoading}, dataCount: ${shortServicesProvider.shortServices.length}, error: ${shortServicesProvider.errorMessage}');
    
    // If no data and not loading, try to load data
    if (shortServicesProvider.shortServices.isEmpty && !shortServicesProvider.isLoading && shortServicesProvider.errorMessage == null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
                    ...
                    child: Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 16),
                          Text(shortServicesProvider.errorMessage!, style: TextStyle(color: Colors.red, fontSize: 16), textAlign: TextAlign.center),
                          SizedBox(height: 16),
                          ElevatedButton(onPressed: () => shortServicesProvider.getAllShortServices(), child: Text('Retry')),
                        ],
                      ),
                    ),
                  )
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        IconButton(icon: Icon(Icons.close, color: Colors.green), onPressed: () => shortServicesProvider.clearMessages()),
                      ],
                    ),
                  ),
                SizedBox(height: 15),
                      ...
                      child: ScrollbarTheme(
                        ...
                        child: LayoutBuilder(
                          builder: (context, constraints) {
                            final double tableWidth = constraints.maxWidth < 1150 ? 1150 : constraints.maxWidth; // expand when screen is larger
                                        ...
                                        children: [
                                          ...
                                          if (shortServicesProvider.shortServices.isNotEmpty)
                                            ...shortServicesProvider.shortServices.asMap().entries.map((entry) {
                                              ...
                                              return TableRow(
                                                decoration: BoxDecoration(color: index.isEven ? Colors.grey.shade100 : Colors.grey.shade50),
                                                children: [
                                                  _buildCell2(
                                                  ...
                                                  _buildPriceWithAdd("AED-", service['pending_payment'] ?? service['cost'] ?? '0'),
              ...
              children: [
                Padding(
                  ...
                  child: Column(
                    ...
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text("EDIT SHORT SERVICE", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black)),
                      Text("Ref ID: ${service['ref_id'] ?? 'N/A'}", style: const TextStyle(fontSize: 12, color: Colors.black54)),
                      const SizedBox(height: 20),
                      CustomTextField1(label: 'CLIENT NAME', text: clientName, onChanged: (val) => clientName = val),
                      const SizedBox(height: 12),

                      CustomTextField1(label: 'COST (AED)', keyboardType: TextInputType.number, text: cost, onChanged: (val) => cost = val),
                      const SizedBox(height: 12),
                          ...
                          ElevatedButton(
                            ...
                            onPressed: () async {
                              if (clientName.isNotEmpty && cost.isNotEmpty && managerName.isNotEmpty) {
                                try {
                                  ...
                                  if (provider.errorMessage == null) {
                                    ...
                                    ScaffoldMessenger.of(
                                      context,
                                    ).showSnackBar(SnackBar(content: Text(provider.errorMessage!), backgroundColor: Colors.red));
                                  }
                                } catch (e) {
                                ...
                                ScaffoldMessenger.of(
                                  context,
                                ).showSnackBar(const SnackBar(content: Text('Please fill all fields'), backgroundColor: Colors.orange));
                              }
                            },
                ...
                Positioned(
                  right: 0,
                  top: 0,
                  child: IconButton(icon: const Icon(Icons.close, size: 25, color: Colors.red), onPressed: () => Navigator.of(context).pop()),
                ),
              ],
            ),
          ),
        ...
        children: [
          ...
          if (copyable)
            GestureDetector(
              ...
              child: Padding(padding: const EdgeInsets.only(left: 8), child: Icon(Icons.copy, size: 12, color: Colors.blue[700])),
            ),
        ],
      ),
    );
